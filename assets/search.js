...
$.widget("custom.catcomplete", $.ui.autocomplete, {
  ...
  _renderMenu: function (ul, items) {
    ...
    $.each(items, (index, item) => {
      const shouldCategoryLabelBeRendered =
        itemHasResults(item) && item.category !== category;
...
const highlight = (match) =>
  `<span class="resultHighlight">` + match + `</span>`;
const escapeHtml = (str) =>
  str.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;");

const labelForPackage = (element) =>
  element.m ? element.m + "/" + element.l : element.l;
...
const renderableFromLabel = (label, regex) =>
  escapeHtml(label).replace(regex, highlight);
...
$(() => {
  $("#search").catcomplete({
    ...
    source: function (request, response) {
      ...
      const camelCaseRegexp = $.ui.autocomplete
        ...
        dataset
          ...
          .forEach((element) => {
            if (exactMatcher.test(element.l)) {
              ...
              element.renderable = renderableFromLabel(
                element.l,
                camelCaseMatcher
              );
              exactOrCamelMatches.push(element);
        ...
        dataset
          ...
          .forEach((element) => {
            ...
            if (exactMatcher.test(label)) {
              element.renderable = renderableFromLabel(
                labelForNested(element),
                new RegExp(exactRegexp, "i")
              );
              exactOrCamelMatches.push(element);
            } else if (camelCaseMatcher.test(label)) {
              element.renderable = renderableFromLabel(
                labelForNested(element),
                new RegExp(camelCaseRegexp)
              );
              exactOrCamelMatches.push(element);
      ...
      const modules = moduleSearchIndex
        ? processWithExactLabel(moduleSearchIndex, constants.labels.modules)
        : [];
      const packages = packageSearchIndex
        ? processPackages(packageSearchIndex)
        : [];
      const types = typeSearchIndex
        ? processNested(typeSearchIndex, constants.labels.types)
        : [];
      const members = memberSearchIndex
        ? processNested(memberSearchIndex, constants.labels.members)
        : [];
      const tags = tagSearchIndex
        ? processWithExactLabel(tagSearchIndex, constants.labels.tags)
        : [];
